...
<head>
    ...
    <title>Edit Transaksi Barang</title>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
          ...
          <!-- End of Topbar -->
                ...
                <div class="container-fluid">
                    ...
                    <!-- Page Heading -->
                    <h1 class="h3 mb-4 text-gray-800">Edit Transaksi Barang</h1>
                    <div class="card shadow mb-4">
                        
                        <div class="card-body">
                        <form action="<?php echo site_url('prosesubahtransaksiict/' . $datab['id_transaksi']) ?>" method="post" enctype="multipart/form-data">
                          <div class="form-group">
                              ...
                              <select name="id_barang" id="id_barang" class="form-control">
                                  ...
                                  <?php foreach ($datub as $key => $value) { ?>
                                      <option value="<?= $value['id_barang']?>" <?= ($value['id_barang'] == $datab['id_barang']) ? 'selected' : '' ?> ><?=$value['kode_barang']?></option>
                                  <?php } ?>
                              </select>
                          </div>
                          <div class="form-group">
                              <label for="jenis_transaksi" style="border-bottom: 3px solid #000;">Jenis Transaksi Barang</label><br>
                              <select name="jenis_transaksi" id="jenis_transaksi" class="form-control">
                                  <option value="" selected disabled>--Option--</option>
                                  <option value="Penyerahan" <?= ($datab['jenis_transaksi'] === 'Penyerahan') ? 'selected' : '' ?>>Penyerahan</option>
                                  <option value="Pengembalian" <?= ($datab['jenis_transaksi'] === 'Pengembalian') ? 'selected' : '' ?>>Pengembalian</option>
                              </select>
                          </div>
                          <div class="form-group">
                              <label for="id_kondisi" style="border-bottom: 3px solid #000;">Kondisi Barang</label><br>
                              <select name="id_kondisi" id="id_kondisi" class="form-control">
                                  <option value="" selected disabled>--Option--</option>
                                  <?php foreach ($datuk as $key => $value) { ?>
                                          <option value="<?= $value['id_kondisi']?>" <?= ($value['id_kondisi'] == $datab['id_kondisi']) ? 'selected' : '' ?>><?=$value['kondisi']?></option>
                                        <?php } ?>
                              </select>
                          </div>
                          <div class="form-group">
                              <label for="nama_penyerah" style="border-bottom: 3px solid #000;">Nama Penyerah</label><br>
                              <select name="nama_penyerah" id="nama_penyerah" class="form-control">
                                  <option value="" selected disabled>--Option--</option>
                                  <?php foreach ($dater as $key => $value) { ?>
                                      <option value="<?= $value['id_pengguna']?>" <?= ($value['id_pengguna'] == $datab['nama_penyerah']) ? 'selected' : '' ?>><?=$value['nama_pengguna']?></option>
                                  <?php } ?>
                              </select>
                          </div>
                          <div class="form-group">
                              <label for="nama_penerima" style="border-bottom: 3px solid #000;">Nama Penerima</label><br>
                              <select name="nama_penerima" id="nama_penerima" class="form-control">
                                  <option value="" selected disabled>--Option--</option>
                                  <?php foreach ($dater as $key => $value) { ?>
                                      <option value="<?= $value['id_pengguna']?>" <?= ($value['id_pengguna'] == $datab['nama_penerima']) ? 'selected' : '' ?>><?=$value['nama_pengguna']?></option>
                                  <?php } ?>
                              </select>
                          </div>
                          <div class="form-group">
                          ...
                          <div class="form-group">
                              <a href="<?=base_url('pengelola-transaksi-barang-ict')?>" class="btn btn-danger" style="float: right; margin-right:10px">Cancel</a>
                          </div>
                      </form>
